...
export async function POST(request: Request) {
  try {
    ...
    if (!text || text.trim().length < 10) {
      return NextResponse.json({ error: 'No se proporciono texto válido' }, { status: 400 })
    }
...
- Responde SOLO con el JSON, sin explicaciones adicionales`
    ...
    if (!responseText) {
      return NextResponse.json({ error: 'No se obteve respuesta de la IA' }, { status: 500 })
    }
    ...
    return NextResponse.json(
      { error: 'Error al procesar el texto. Intenta de nuevo.' },
      { status: 500 }
    )
  }
}